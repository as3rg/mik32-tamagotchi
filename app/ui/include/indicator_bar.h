...
protected:
  constexpr void draw_impl(screen::canvas &screen, size_t offset_y, size_t offset_x) const override {
    ...
    {
      size_t target_y =
          offset_y + height() - lvl_thickness - (height() - lvl_thickness) * lvl_1() / max_value;
      dash_line(screen, target_y, offset_x, offset_x + width(), lvl_thickness, lvl_dash_len);
    }
    {
      size_t target_y =
          offset_y + height() - lvl_thickness - (height() - lvl_thickness) * lvl_2() / max_value;
      dash_line(screen, target_y, offset_x, offset_x + width(), lvl_thickness, lvl_dash_len);
    }
    {
      size_t target_y = offset_y + height() - indicator_thickness -
  ...
  constexpr static void dash_line(screen::canvas &screen, size_t y, size_t x1,
                           size_t x2, size_t thickness, size_t dash_len) {
    size_t x = x1;
