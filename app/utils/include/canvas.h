...
namespace utils {
enum class canvas_orientation { HORIZONTAL, VERTICAL };

template <size_t height_, size_t width_,
          canvas_orientation orientation_ = canvas_orientation::VERTICAL>
struct canvas {
private:
...
public:
  ...
  constexpr canvas(std::initializer_list<std::initializer_list<bool>> ini)
      : canvas(init_storage(ini)) {}
  ...
  constexpr canvas_orientation orientation() const { return orientation_; }

  constexpr typename storage_t::storage_type *raw_data() { return data.data; }

  constexpr const typename storage_t::storage_type *raw_data() const {
    return data.data;
  }

  constexpr const size_t raw_size() const { return data.raw_size(); }

  template <size_t other_height, size_t other_width,
            canvas_orientation other_orientation>
  constexpr canvas &
  draw(const canvas<other_height, other_width, other_orientation> &other,
       size_t y, size_t x, bool invert = false) {
  ...
  constexpr canvas &fill_rectangle(size_t y, size_t x, size_t h, size_t w,
                                   bool color = true) {
  ...
  constexpr canvas &draw_rectangle(size_t y, size_t x, size_t h, size_t w,
                                   size_t thickness = 1, bool color = true) {
...
private:
  ...
  init_storage(std::initializer_list<std::initializer_list<bool>> ini) {
    canvas other;
    auto it_rows = ini.begin();
    for (size_t j = 0; j < other.height() && it_rows != ini.end();
         ++j, ++it_rows) {
      auto it_cols = it_rows->begin();
      for (size_t i = 0; i < other.width() && it_cols != it_rows->end();
           ++i, ++it_cols) {
        other(j, i) = *it_cols;
      }
    }
