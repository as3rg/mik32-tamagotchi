...
template <typename T, typename Tag_ = list_default_tag>
class list;
...
protected:
  ...
  constexpr list_element_base(const list_element_base&) noexcept : list_element_base() {}

  constexpr list_element_base& operator=(const list_element_base& other) noexcept {
    if (&other != this) {
      unlink();
    }
    return *this;
  ...
  constexpr list_element_base(list_element_base&& other) noexcept : list_element_base() {
    *this = std::move(other);
  }
  ...
  constexpr static void link(list_element_base* first, list_element_base* second) noexcept {
    first->next = second;
    second->prev = first;
  }
};
...
class list {
  ...
  private:
    constexpr explicit list_iterator(const list_element_base* el) : element(const_cast<list_element_base*>(el)) {}
  ...
  public:
    ...
    constexpr friend bool operator==(const list_iterator& left, const list_iterator& right) noexcept {
    ...
    constexpr friend void swap(list_iterator& left, list_iterator& right) noexcept {
...
private:
  constexpr static list_element_base* get_base_ptr(const_iterator& it) noexcept {
